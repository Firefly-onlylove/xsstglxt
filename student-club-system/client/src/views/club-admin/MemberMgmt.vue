...
async function setManager(row) {
  ...
  const res = await api.post('/api/club/' + clubId.value + '/members/' + row.member_id + '/appoint', { role: 'vice_president' })
...
async function demoteToMember(row) {
  ...
  const res = await api.post('/api/club/' + clubId.value + '/members/' + row.member_id + '/appoint', { role: 'member' })
