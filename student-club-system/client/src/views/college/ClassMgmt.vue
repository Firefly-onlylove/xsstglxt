...
const fields = computed(() => [
  { prop: 'name',     label: '班级名称', placeholder: '如 软件工程2301班' },
  { prop: 'major_id', label: '所属专业', type: 'select',
    options: majors.value.map(m => ({ label: m.name, value: m.major_id })) }
])
...
function openEdit(row) { editing.value = row; formModal.value.open({ name: row.name, major_id: row.major_id }) }
