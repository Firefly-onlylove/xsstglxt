...
/* GET /api/college/students?major_id=&class_id=&search= */
void col_student_list(ApiContext *ctx) {
    ...
    /* 动态拼可选过滤条件。用户输入 search 需转义。 */
    char where[512];
    int off = snprintf(where, sizeof(where),
                       "u.college_id=%d AND u.role IN ('general_student','club_member','club_admin')",
                       cid);
    if (major_id > 0)
    ...
    MYSQL_RES *res = db_query(
        ...
        "WHERE %s ORDER BY u.student_no", where);
