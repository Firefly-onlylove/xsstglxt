...
void pub_notif_list(ApiContext *ctx) {
    ...
    char filter[128];
    if (unread_only)
        snprintf(filter, sizeof(filter), "receiver_id=%d AND is_read=0", uid);
    else
        snprintf(filter, sizeof(filter), "receiver_id=%d", uid);
...
void pub_notif_unread(ApiContext *ctx) {
    ...
    int cnt = db_query_int(
        "SELECT COUNT(*) FROM notifications WHERE receiver_id=%d AND is_read=0", uid);
...
void pub_notif_read(ApiContext *ctx) {
    ...
    db_execute("UPDATE notifications SET is_read=1, read_at=NOW() "
               "WHERE notification_id=%d AND receiver_id=%d", nid, uid);
    api_ok_msg(ctx, "已读");
}
...
void pub_notif_read_all(ApiContext *ctx) {
    ...
    db_execute("UPDATE notifications SET is_read=1, read_at=NOW() "
               "WHERE receiver_id=%d AND is_read=0", uid);
    api_ok_msg(ctx, "全部已读");
}
